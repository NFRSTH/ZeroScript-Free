...
const ZSProvider = (() => {
  ...
  async function typeAndSend(text,images){ const ed=getEditor(); if(!ed) throw new Error('Claude input not found'); ed.focus(); document.execCommand('selectAll',false,null); document.execCommand('insertText',false,text); await waitFor(()=>{ const b=document.querySelector(S.sendBtn); return b && !b.disabled; },800); const b=document.querySelector(S.sendBtn); if(b) b.click(); else ed.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',bubbles:true})); }
